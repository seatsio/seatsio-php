...
class ExponentialBackoffTest extends SeatsioClientTest
{
    ...
    public function testAbortsEventuallyIfServerKeepsReturning429()
    {
        ...
        try {
            ...
            self::assertEquals("GET " . self::httpbinUrl() . "/status/429 resulted in a `429 Too Many Requests` response. Body: ", $exception->getMessage());
            $waitTime = time() - $start;
    ...
    public function testAbortsDirectlyIfServerReturnsOtherErrorThan429()
    {
        ...
        try {
            ...
            self::assertEquals("GET " . self::httpbinUrl() . "/status/400 resulted in a `400 Bad Request` response. Body: ", $exception->getMessage());
            $waitTime = time() - $start;
    ...
    public function testAbortsDirectlyIfServerReturns429ButMaxRetries0()
    {
        $start = time();
        try {
            $client = new SeatsioClient(Region::withUrl(self::httpbinUrl()), "aSecretKey", null, 0);
            $client->client->get("/status/429")->getBody();
            throw new \Exception("Should have failed");
        } catch (SeatsioException $exception) {
            self::assertEquals("GET " . self::httpbinUrl() . "/status/429 resulted in a `429 Too Many Requests` response. Body: ", $exception->getMessage());
